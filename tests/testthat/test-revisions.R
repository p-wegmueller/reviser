...
# Tests for get_revisions, get_first_efficient_release, get_revision_analysis, and related functions
...
dates <- seq.Date(from = as.Date("2020-01-01"), by = "month", length.out = n_obs)

# Create pub_date based long format (for get_revisions and most tests)
df_long_rev <- data.frame(
  time = rep(dates, n_vintages),
  pub_date = rep(seq.Date(as.Date("2020-02-01"), by = "month", length.out = n_vintages), each = n_obs),
  value = as.vector(sapply(1:n_vintages, function(v) {
...
test_that("get_revisions returns correct structure with interval", {
  ...
  expect_true(inherits(result, "tbl_pubdate") || inherits(result, "tbl_revision"))
...
test_that("get_revisions handles nth_release parameter", {
  ...
  df_with_pub <- df_release %>%
    dplyr::group_by(time) %>%
    dplyr::mutate(pub_date = seq.Date(as.Date("2020-02-01"), by = "month", length.out = dplyr::n())[dplyr::row_number()]) %>%
    dplyr::ungroup()
...
test_that("full revision workflow works", {
  ...
  expect_true(inherits(revisions, "tbl_pubdate") || inherits(revisions, "tbl_revision"))
