...
#' @examples
#' # Example usage:
#' df <- get_nth_release(tsbox::ts_span(tsbox::ts_pc(reviser::gdp_us), start = "1980-01-01"), n = 0:1)
...
kk_nowcast <- function(
  ...
  for (j in (e):0) {
    ...
    X_lag[, (e+1) - j] <- dplyr::lag(dplyr::pull(df[paste0("release_",e)]), j+1)
  }
  ...
  if (h > 0) {
    ...
    forecast[2,] <-  sur_ss_mat$Tmat %*% kalman$filtered_states[nrow(kalman$filtered_states),]
    if (h > 1)  {                      
      for (hh in 2:(h)) {
    ...
    # Forecasted states
    forecast_states <- tibble::tibble(as.data.frame(forecast[1:(h+1),1:(e+1)])) %>%
      stats::na.omit() %>%
      dplyr::mutate(time = forecast_dates) %>%
      dplyr::select(time, !!!stats::setNames(seq_along(state_names), state_names))
    
    # Forecasted observations
    forecast_observation <- tibble::tibble(as.data.frame(forecast[1:(h+1),])) %>%
      stats::na.omit()
    forecast_observation <- forecast_observation  %>%
      dplyr::mutate(dplyr::across(
        .cols = 1:(ncol(forecast_observation) - (e+1)),  # Columns to sum (e.g., 1 to n-e)
        .fns = ~ . + forecast_observation[[which(names(forecast_observation) == dplyr::cur_column()) + e]], 
...
kk_matrices <- function(e, model, params = NULL, type = "numeric") {
  ...
  H <- array(0,c(e + 1, e + 1))
  for (jj in 2:(e+1)) {
    H[jj, jj] <-  ifelse(type == "numeric", (params[ii]), paste0("eps", e+1-jj)) 
...
kk_to_ss <- function(II, FF, GG, R, H, epsilon = 1e-6) {
  ...
  W[((e+2):(2*(e+1))), ((e+2):(2*(e+1)))] <- H[1:(e+1),1:(e+1)] + (II-GG)%*%v_t_2%*%t(II-GG)
...
kalman_filter_smoother <- function(Y, FF, V, GG, W, m0, C0) {
  ...
  for (t in (n - 1):1) {
    ...
    P_smooth[, , t] <- P_filtered[, , t] + Jt %*% (P_smooth[, , t + 1] - Pt[, , t + 1]) %*% t(Jt)
  }
