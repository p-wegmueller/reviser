...
dta9 <- get_revision_analysis(
  dta %>% filter(year(pub_date) %in% c(2014:2023)),
...
actl <- tsbox::ts_wide(dta10$CH$data)$release_3[
  (dta10$CH$e + 1):nrow(tsbox::ts_wide(dta10$CH$data))
]
...
# RMSE first release
rmse2 <- sqrt(mean(
  (tsbox::ts_wide(dta10$CH$data)$release_0[
    (dta10$CH$e + 1):nrow(tsbox::ts_wide(dta10$CH$data))
  ] -
    actl)^2,
  na.rm = T
...
# RMSE first release
rmse2 <- sqrt(mean(
  (ts_wide(dta12$data)$release_0[(dta12$e + 1):nrow(ts_wide(dta12$data))] -
    actl)^2,
  na.rm = T
))
...
plot_vintages(
  dta4 %>% filter(id == "CH"),
  dim_col = "release",
  title = "Swiss GDP releases"
)
plot_vintages(
  dta4 %>% filter(id == "USA"),
  dim_col = "release",
  title = "US GDP releases"
)

plot_vintages(
  dta5 %>% select(id, value, time = pub_date, pub_date = time),
  dim_col = "id",
  title = "GDP growth estimates",
  subtitle = "For the 2007-10-01 value"
)

plot_vintages(
  dta6 %>% filter(id == "CH", pub_date == "2024-07-01"),
  type = "bar",
...
plot_vintages(
  dta6 %>%
    filter(
      id == "CH",
      time >= "2020-01-01" & time < "2022-01-01" & pub_date < "2022-01-01"
    ),
  type = "line",
...
plot_vintages(
  dta7 %>% filter(id == "CH", time >= "2020-01-01" & time < "2022-01-01"),
  type = "point",
  title = "Revisions to Swiss GDP growth rates during Covid-19",
...
plot_vintages(
  dta8 %>% filter(id == "CH", pub_date == "2024-07-01"),
  type = "bar",
  title = "Revisions to Swiss GDP growth rates",
