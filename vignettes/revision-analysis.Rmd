...
In the above examples, we compare the final release of GDP with the first and second release (diagonal of the real-time data matrix). However, it is also possible to compare revisions between given publication dates. In the example below, we compare revisions between GDP releases in April and July 2024 and the final release from October 2024. The grouping of the statistics takes place over the `pub_date` or `release` column.
