...
#### 1. **The Noise Test**
  ...
  - The function tests the joint hypothesis (`"noise_wald_p_value"`) that both \( \alpha = 0 \) and \( \beta = 0 \) using a **heteroskedasticity and autocorrelation consistent (HAC) covariance matrix** to ensure robust inference.
  - \( \alpha = 0 \) tests (`"noise_intercept_p_value"`) whether the revisions have a systematic bias.
  - \( \beta = 0 \) tests (`"noise_coeff_p_value"`) whether the revisions are correlated with the final value, indicating inefficiency.
...
\]
  ...
  - The function tests the joint hypothesis (`"news_wald_p_value"`) that both \( \alpha = 0 \) and \( \beta = 0 \) using a **HAC covariance matrix** to ensure robust inference.
  - \( \alpha = 0 \) tests (`"news_intercept_p_value"`) whether the revisions have a systematic bias.
  - \( \beta = 0 \) tests (`"news_coeff_p_value"`) whether the revisions are correlated with the initial value, indicating inefficiency.
...
However, U1 suffers from **several limitations**, as highlighted by **Granger & Newbold (1973)**. A critical issue arises when **one of \( Y_t^h \) or \( Y_t^f \) equals zero in all periods**. In such cases, the denominator and numerator become equal, leading to **U1 = 1** even when preliminary estimates are close to the final values.  

#### 2. **Theil’s U2 Statistic**  

To address these shortcomings, Theil (1966) proposed an alternative measure, **U2**, which is defined as:  
...
- **\( U_2 = 1 \)** means the accuracy of the preliminary estimates is equal to a **naïve random walk**.  
