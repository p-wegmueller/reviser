...
plot_vintages(
  revisions_interval %>% 
    ...
    get_nth_release(0:1),
  dim_col = "release",
  title = "Revisions of 2nd (release_0) and 3rd (release_1) release",
  subtitle = "For the US"
)
...
Instead of comparing consecutive releases, we can compare each data point against its nth release. For example, if we want to evaluate revisions between the first and second releases:
...
This is useful for assessing how much initial estimates deviate from later, more refined estimates.

These visualizations can help detect bias in revisions, identify persistent trends, and inform decision-making.

## Conclusion

The `get_revisions()` function in the `reviser` package provides a flexible way to analyze time series data revisions. By allowing users to compute revisions using different methods, it helps assess the reliability of early estimates and understand the dynamics of data revisions over time.

For further analysis, users can integrate `get_revisions()` with other functions in the `reviser` package, such as summary statistics and revision diagnostics (`get_revision_analysis()`), to gain deeper insights into the revision process.
